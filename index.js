...
// ============================================================
// ⚙️ CẤU HÌNH - GIỮ NGUYÊN HOÀN TOÀN NHƯ BAN ĐẦU (Cập nhật thương hiệu Hoàng)
// ============================================================
...
const ADMIN_USERNAME = "@cskhvilong1"; // ✅ ĐÃ CẬP NHẬT THEO THƯƠNG HIỆU MỚI "BY HOÀNG"[cite: 1]
...
// ============================================================
// 🧠 THUẬT TOÁN - GIỮ NGUYÊN 100% GỐC + NÂNG CẤP SIÊU SÂU MỚI
// ============================================================
class HashAnalyzer {
  constructor() {
    this.history = [];
    this.breakProtector = { mode:null, count:0, consecutiveWrong:0, adaptiveThreshold:3, lastPrediction:null, reverseCount:0 };
    this.coreState = { daoChieu:false, entropyHigh:false }; // ✅ ĐÃ LOẠI BỎ HOÀN TOÀN NHÃN AI
    this.md5Stats = {
      byteTransition: Array.from({length:256},()=>[0,0]),
      bitRun: [], totalSamples:0, taiReal:0, xiuReal:0,
      markov2: Array.from({length:4},()=>[0,0]), // 00,01,10,11
      bitEntropyLog: [], accuracyLog: []
    };
    this.lastDetails = null; // Lưu chi tiết phân tích của lượt chạy gần nhất
  }
  ...
  // ============================================================
  // 💥 6 LỚP THUẬT TOÁN NÂNG CẤP CHUYÊN SÂU MỚI (TUYỆT ĐỐI KHÔNG RANDOM)
  // ============================================================
  
  // LỚP 1: Phân Tích Chu Kỳ Phổ (Fourier Spectrum Approximation)
  ...
  // LỚP 2: Điểm Tích Lũy Bất Đối Xứng (Cusum Drift Path)
  ...
  // LỚP 3: Độ Phức Tạp nén Lempel-Ziv (LZ Complexity Metric)
  ...
  // LỚP 4: Tự Tương Quan Khoảng Cách Hamming (Hamming Distance Autocorrelation)
  ...
  // LỚP 5: Kiểm Định Tỷ Số Phương Sai Luỹ Tiến (Variance Ratio Test)
  ...
  // LỚP 6: Xác Suất Hậu Nghiệm Bayes (Bayesian Conditional Probability Network)
  ...
  // ============================================================
  // 🧬 THUẬT TOÁN PHÂN TÍCH BỐC TÁCH HEX, ENTROPY, BIT & DEEP SCORE (MỚI)
  // ============================================================
  
  // Phân bổ tần suất ký tự Hexadecimal & Entropy mật độ mật mã
  ...
  // Thuật toán chấm điểm Deep Score đa biến tổng hợp phổ biến (Không ngẫu nhiên)
  calculateDeepScore(bytes, hexEnt, fourier, cusum, autoCor, varRatio, bayes) {
    ...
    const score = (byteScore * 0.2) + (fPower * 0.25) + (bayesConf * 0.35) + (entScore * 0.2);
    return Math.min(97, Math.max(53, Math.round(score * 1.25)));
  }
  ...
  superEnsemble(h,md5){
    ...
    // Giữ nguyên tính toán entropy cũ
    ...
    // TÍCH HỢP 6 LỚP PHÂN TÍCH TOÁN HỌC MỚI (TỰ ĐỘNG LƯU CHI TIẾT)
    ...
    // Lưu trữ thông số phân tích chi tiết để vẽ đồ thị / hiển thị phần trăm
    this.lastDetails = { fourier, cusum, lz, autoCor, varRatio, bayes, T, X };

    // Áp dụng bộ nhân co giãn từ độ phức tạp Lempel-Ziv lên các phương thức phụ thuộc chuỗi tuần hoàn
  ...
  deepEngine(h,p){ // ✅ ĐÃ ĐỔI TÊN ĐỂ TRIỆT TIÊU HOÀN TOÀN NHÃN AI KHỎI HỆ THỐNG
    if(h.length<8||!p) return p;
    const r=h.slice(0,20).map(s=>s.result==='Tài'?1:0);
    const en=this.calcEntropy(r.slice(0,8));
    let sk=1; for(let i=1;i<r.length;i++) if(r[i]===r[0])sk++; else break; // ✅ ĐÃ SỬA LỖI BIẾN CHƯA KHAI BÁO (st++ thành sk++) ĐỂ KHÔNG BỊ LỖI PHÂN TÍCH
  ...
  analyze(hex){
    ...
    // Tính toán tỷ lệ phần trăm chính xác nâng cấp bằng thuật toán Deep Score & Hex Entropy thực tế
    ...
    if(pred==="Tài"){ res="🔥 TÀI"; ic="🔥"; }
      ...
      // Logic fallback gốc để bảo đảm không lỗi trong mọi trường hợp bất thường
...
bot.on('message:text', async ctx=>{
  ...
  // Tạo thanh bar hiển thị tiến trình (Tokyo Night neon style)
...
━━━━━━━━━━━━━━━━━━━━━━━
🔮 <b>HỆ THỐNG DỰ ĐOÁN</b>
👉 <b>${r.res}</b>  (Độ tin cậy: <b>${r.conf}%</b>)
...
🧬 <b>6 LỚP PHÂN TÍCH CHUYÊN SÂU</b>
...
// ============================================================
// 🚀 SERVER KEEP‑ALIVE → ĐỘC LẬP HOÀN TOÀN KHÔNG BAO GIỜ NGỦ
// ============================================================
const app = express();
...
// TỰ PING ĐỂ GIỮ SERVER LUÔN HOẠT ĐỘNG
setInterval(async ()=>{
  try{ await fetch(`http://localhost:${PORT}/ping`).catch(()=>{}); }catch{}
}, 4*60*1000);
